...
class BiliSpider:
            ...
            with open(self.output_file, 'r', encoding='utf-8') as f:
                for line in f:
                    try:
                        ...
                        mid = item.get('card', {}).get('mid') or item.get('mid')
    ...
    async def fetch(self, uid, engine):
        """执行单次请求"""
        try:
            api_cfg = {"url": "https://api.bilibili.com/x/web-interface/card", "method": "GET", "wbi": True}
            res = await Api(**api_cfg, credential=engine["cred"]).update_params(mid=uid).result
            return "OK", res
...
if __name__ == '__main__':
    ...
    spider = BiliSpider(
        ...
        output_file="data/fanshi_user_card_test.jsonl",
        cookie_file="bili_cookies_B.json",
        engine_count=30
    )
